...
export async function listLists(
  ...
  context: ToolContext
): Promise<ToolResult<{ lists: Array<{ id: string; name: string; created_at: string; updated_at: string; protected?: boolean }> }>> {
  try {
    // Validate input
    ...
    const { data: lists, error } = await supabase
      .from('grocery_lists')
      .select('id, name, created_at, updated_at, protected')
      .eq('household_id', context.householdId)
